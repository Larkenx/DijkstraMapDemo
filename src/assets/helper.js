...
export function getVisibleTiles(location, range, map) {
	let { x, y } = location
	...
	let visibleTiles = []
	fov.compute(x, y, range, (fx, fy, r, visibility) => {
		visibleTiles.push({ x: fx, y: fy })
	})
	return visibleTiles
}

export function createFovDijkstraMap(start, notVisibleTiles, blockedPredicate) {
	...
	for (let coord of Object.keys(notVisibleTiles)) {
		console.assert(!(coord instanceof String))
